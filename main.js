...
function introduceTheActors (actorArray) {
    ...
    for (let i = 0; i < actorArray.length; i++) {
        statementArray[i] = "Hi, I'm " + actorArray[i].name + " and I was in " + actorArray[i].movie + ".";
    }
...
techCompanies.storeFounders = function (leadership) {
    const leaders = [];
    for (const company in leadership) {
        if (typeof leadership[company] == 'string') {
            leaders.push(leadership[company]);
        }
    }
    return leaders;
}
